...
def Upsample(dim):
    return nn.ConvTranspose3d(dim, dim, kernel_size=2,stride=2, padding=0)


def Downsample(dim):
    return nn.Conv3d(dim, dim, kernel_size=3, stride=2, padding=1)
...
class FeatureFusion(nn.Module):
    def __init__(self, in_channels, condition_channels, dims=3):
        super().__init__()
        self.fusion_conv = nn.Conv3d(in_channels + condition_channels, in_channels, 
...
class Unet3D(nn.Module):
    def __init__(
        ...
        resnet_groups=8,
        has_self_cond=False,
        use_multi_scale_cond = False,
        multi_scale_cond_dim=128,
    ):
        super().__init__()
        self.channels = channels

        # temporal attention and its relative positional encoding
        self.use_multi_scale_cond = use_multi_scale_cond
        ...
        self.time_rel_pos_biases = nn.ModuleList([])
        seq_length = 32 
        self.time_rel_pos_bias_init = RelativePositionBias(
            heads=attn_heads, max_distance=seq_length)
        #pdb.set_trace()
        for _ in range(len(dim_mults)):
            self.time_rel_pos_biases.append(RelativePositionBias(heads=attn_heads, max_distance=seq_length))
            print(" seq length : " , seq_length)
            seq_length = seq_length // 2  # 每次下采样后长度减半
                # 确保长度不小于1
        ...
        self.init_conv = nn.Conv3d(channels, init_dim, (1, 1,
                                   1), padding=(0, 0, 0))
        ...
        pdb.set_trace()
        if use_multi_scale_cond:
            self.fusion_blocks = nn.ModuleDict()
            for level , (dim_in , _ ) in enumerate(in_out[:-1]):
                self.fusion_blocks[f'level_{level}'] = FeatureFusion(dim_in, multi_scale_cond_dim, dims=3)
        # layers
        pdb.set_trace()
    ...
    def forward(
        ...
        assert not (self.has_cond and not exists(cond)
               ...
               # 检查多尺度特征是否提供
        if self.use_multi_scale_cond:
            assert exists(multi_scale_features), 'multi_scale_features must be provided when use_multi_scale_cond=True'
            assert len(multi_scale_features) >= len(self.downs) - 1, 'not enough multi_scale_features provided'
        ...
        time_rel_pos_bias_init = self.time_rel_pos_bias_init(x.shape[2], device=x.device)
        ...
        x = self.init_temporal_attn(x, pos_bias=time_rel_pos_bias_init)
        ...
        h = []
        #pdb.set_trace()
        for level ,(block1, block2, spatial_attn, temporal_attn, downsample) in enumerate(self.downs):
            #print("level : " , level)
            current_time_rel_pos_bias = self.time_rel_pos_biases[level](x.shape[2], device=x.device)
            #pdb.set_trace()
            if self.use_multi_scale_cond and level < len(multi_scale_features):
                x = self.fusion_blocks[f"level_{level}"](x,multi_scale_features[level])
            #pdb.set_trace()
            x = block1(x, t)
            x = block2(x, t)
            x = spatial_attn(x)
            x = temporal_attn(x, pos_bias=current_time_rel_pos_bias,
                              focus_present_mask=focus_present_mask)
            #print("down x " , x.shape)
            h.append(x)
            x = downsample(x)
        mid_time_rel_pos_bias = self.time_rel_pos_biases[len(self.downs)-1](x.shape[2], device=x.device)
        #print( " mid x " , x.shape)
        x = self.mid_block1(x, t)
        x = self.mid_spatial_attn(x)
        x = self.mid_temporal_attn(
            x, pos_bias=mid_time_rel_pos_bias, focus_present_mask=focus_present_mask)
        x = self.mid_block2(x, t)

        for idx , (block1, block2, spatial_attn, temporal_attn, upsample )in enumerate(self.ups):
            #pdb.set_trace()
            up_level = len(self.downs) - idx - 1
            current_time_rel_pos_bias = self.time_rel_pos_biases[up_level](x.shape[2], device=x.device)
